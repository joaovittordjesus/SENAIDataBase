...
CREATE TABLE    cliente(codcli char(3) NOT NULL PRIMARY KEY,
...
CREATE TABLE    venda (duplic char(6) not null primary key,
...
SELECT *
FROM VENDA; -- INSERÇÃO DE DADOS NA TABELA CLIENTE
...
--SELECT COM ALIAS(ABREVIAÇÃO/APELIDO) E ORDER BY
...
FROM CLIENTE
INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
WHERE CLIENTE.NOME LIKE 'PCTEC - MICROCOMPUTADORES S/A.';
--OU
SELECT VENDA.DUPLIC,
       CLIENTE.NOME
FROM CLIENTE
...
FROM CLIENTE
INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
WHERE VENDA.VENCIO >= '2004-11-01' AND VENDA.VENCIO < '2004-12-01'
ORDER BY VENDA.VENCIO ASC;
--OU
SELECT  CLIENTE.NOME,
        VENDA.VENCIO,
        VENDA.DUPLIC
FROM    CLIENTE
INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
WHERE   EXTRACT(YEAR FROM VENDA.VENCIO) = 2004 AND EXTRACT(MONTH FROM VENCA.VENCIO) = 11
ORDER BY    VENDA.VENCIO ASC;
...
FROM    CLIENTE 
INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
WHERE EXTRACT(MONTH FROM VENDA.VENCIO) = 10
ORDER BY VENDA.VENCIO ASC;
...
--5
SELECT CLIENTE.NOME, COUNT(*) AS TITULOS, SUM(VENDA.VALOR)
FROM CLIENTE INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
GROUP BY CLIENTE.NOME;
...
SELECT  CLIENTE.NOME AS CLIENTE,
        VENDA.VALOR,
        ROUND(VENDA.VALOR * 0.10, 2) AS JUROS,
        ROUND(VENDA.VALOR * 1.10, 2) AS TOTAL
FROM    CLIENTE
INNER JOIN VENDA ON CLIENTE.CODCLI = VENDA.CODCLI
ORDER BY CLIENTE;

--ATIVIDAE COMPLEMENTAR SQL JOINS
CREATE DATABASE AT5;
